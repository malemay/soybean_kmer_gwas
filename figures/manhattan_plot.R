...
program_labels <- c("Platypus", "vg", "Paragraph", "kmers")
...
pushViewport(viewport(height = 0.95, y = 0, just = "bottom", layout = grid.layout(nrow = 4)))
...
for(i in 1:4){
	...
	if(programs[i] == "kmers") {
		grid.text(expression(paste("(d) ", italic(k)-mers)), 0.1, 1.08, hjust = 0, gp = gpar(fontsize = 14))
	} else {
