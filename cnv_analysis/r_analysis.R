# This script aims to find out whether there is copy number variation
# at the R locus. This will done in the following steps:
...
gene_name <- tolower("Glyma.09G234900")
...
if(interactive()) {
	covplot <- plot_coverage(bam_files, average_coverage = average_cov,
				 yscale = c(0, 4), cores = 30,
				 #grange = GRanges(seqnames = "Gm09", ranges = IRanges(start = 46120000, end = 46160000)))
				 #grange = genes[gene_name])
				 grange = genes[tolower("Glyma.09G235100")])
}
