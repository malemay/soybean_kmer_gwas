...
# region at the cdwGm15 locus. This will done in the following steps:
...
# A deletion associated with the phenotype was observed at Gm15:37404782-37405538. Let us have a look
...
if(interactive()) {
	covplot <- plot_coverage(bam_files, average_coverage = average_cov,
				 yscale = c(0, 5), cores = 30,
				 grange = GRanges(seqnames = "Gm15", ranges = IRanges(start = 37404782, end = 37405538)))
}
...
# Trying to see if there is a correlation between copy number and resistance to Phytopththora sojae
phenotypic_data <- read.table("phenotypic_data/phenotypic_data.csv", header = TRUE, sep = ";")
cdw <- phenotypic_data[match(names(normalized_cov), phenotypic_data$bayer_id), "corrected_dry_weight"]

boxplot_data <- data.frame(coverage = normalized_cov,
			   cdw = cdw,
			   stringsAsFactors = FALSE)

boxplot_data$deletion <- boxplot_data$coverage < 0.2
boxplot(cdw ~ deletion, data = boxplot_data)
t.test(cdw ~ deletion, data = boxplot_data)
