...
%DIF ADD additional_files/manuscript.tex             Tue Jul  4 19:58:23 2023
...
loci when filtered markers identified significant associations (more details
...
\DIFaddbegin \DIFadd{Only }\textit{\DIFadd{k}}\DIFadd{-mers that occurred twice in a given sample were considered to
reduce the impact of sequencing errors.
...
\ref{seed-coat-color-main-figure} and Supplemental Figures
S16 and
...
study, we noticed apparently spurious associations with }\textit{\DIFadd{k}}\DIFadd{-mers
scattered throughout the genome were common. This was especially the case for
highly significant loci whose signal spanned several megabases such as }\textit{\DIFadd{W}}
\DIFadd{and }\textit{\DIFadd{T}}\DIFadd{. A look at the Q-Q plots derived from }\textit{\DIFadd{k}}\DIFadd{-mer-based GWAS
showed that these usually had the same shape as the Q-Q plots derived from SNP-
...
span a large interval, it is indeed be highly likely that some of the }\textit{\DIFadd{k}}\DIFadd{-mers
coming from the region map elsewhere in the genome. This interpretation is supported
by the fact that the LD between presumably spurious }\textit{\DIFadd{k}}\DIFadd{-mers and
}\textit{\DIFadd{k}}\DIFadd{-mers from target loci was generally high (e.g. Figures S5 and S47).
Overall, the risk of false association due to population structure and the identification
of an appropriate significance threshold seems well controlled, but work remains
to be done in reducing the occurrence of spurious associations.
}

\DIFadd{There are obvious limitations to the relying on the }\DIFaddend presence/absence of
...
\DIFaddbegin \DIFadd{In addition to the link between }\textit{\DIFadd{k}}\DIFadd{-mer presence/absence and sequence
variation, one may wonder to what extent errors may occur in declaring the
...
quantify this error, we compared the number of times that a given position was
...
a proportion of only 0.5 to 1.5\% of positions covered by only one read.
...
.
The robustness of }\textit{\DIFadd{k}}\DIFadd{-mer presence/absence to sequencing depth is also
...
particular type of SV and on the species. \DIFdelbegin \DIFdel{A }\DIFdelend \DIFaddbegin \DIFadd{Until more robust SV calling and genotyping
methods are developed or advances in sequencing technology result in more
trustworthy SV datasets, a }\DIFaddend more efficient approach than
