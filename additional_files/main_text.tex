...
We analyzed a total of 13 traits using three GWAS approaches that differed in
the genotypes used. The first approach used SNP and indel
genotypes called by Platypus. Despite both SNPs and indels being included in
this approach, we will refer to this approach as SNP-based for simplicity. Our
SV-based workflow used various sources of data for SV discovery and a single
program (Paragraph) for genotyping SVs at population scale. Finally, the third
GWAS approach relied on the presence/absence of \textit{k}-mers as genotypes.
Because of the large number of \textit{k}-mers analyzed, we only report results
(e.g. in Manhattan plots) for those that were considered significantly
