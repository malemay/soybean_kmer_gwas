...
Only \textit{k}-mers that occurred twice in a given sample were considered to
...
study, we noticed apparently spurious associations with \textit{k}-mers
scattered throughout the genome were common. This was especially the case for
highly significant loci whose signal spanned several megabases such as \textit{W}
and \textit{T}. A look at the Q-Q plots derived from \textit{k}-mer-based GWAS
...
span a large interval, it is indeed be highly likely that some of the \textit{k}-mers
...
\textit{k}-mers from target loci was generally high (e.g. Figures S5 and S47).
...
There are obvious limitations to the relying on the presence/absence of
...
In addition to the link between \textit{k}-mer presence/absence and sequence
...
The robustness of \textit{k}-mer presence/absence to sequencing depth is also
demonstrated by \shortciteA{gaurav2022}.
